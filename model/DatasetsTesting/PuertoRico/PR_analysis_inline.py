...
df_bm = df[df['biomass'] > 0]
df_no_bm = df[df['biomass'] == 0.0]

(r1, g1, b1) = (df_bm['avg_red'].mean(), df_bm['avg_green'].mean(), df_bm['avg_blue'].mean())
(r2, g2, b2) = (df_no_bm['avg_red'].mean(), df_no_bm['avg_green'].mean(), df_no_bm['avg_blue'].mean())


a = np.zeros((64, 64, 3), dtype=int)
a[:, :, 0] = r1
a[:, :, 1] = g1
a[:, :, 2] = b1

print("Average color of land WITH biomass (i.e. biomass > 0.0)")
plt.imshow(a)
plt.show()

b = np.zeros((64, 64, 3), dtype=int)
b[:, :, 0] = r2
b[:, :, 1] = g2
b[:, :, 2] = b2

print("Average color of land WITHOUT (i.e. biomass = 0.0)")
plt.imshow(b)
plt.show()
...
def average_rgb(image, x, y, subsquare_width):
    shape = image.shape
    width = shape[1]
    height = shape[0]
    i = subsquare_width // 2
