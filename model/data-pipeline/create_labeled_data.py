...
def get_labeled_data(rgb_array, metadata_filepath, conus_data_filepath, pixel_radius=5):
    ...
    jpg_width = rgb_array.shape[1]
    jpg_height = rgb_array.shape[0]
    ...
    for x in range(0, max_x):
        ...
        for y in range(0, max_y):
            ...
            try:
                np.count_nonzero(rgb_array[jpg_y, jpg_x])
            except:
                print(f"Error {y}, {max_y}, {jpg_height}")
                print(rgb_array.shape)

            if np.count_nonzero(rgb_array[jpg_y, jpg_x]) != 0:
                s = rgb_array[jpg_y-pixel_radius:jpg_y+pixel_radius,
...
# Produces a np array of training data for only the colored portions of the image
# Example instance produced in data: [averageR, averageG, averageB, conusLabel]
def get_labeled_data1(satellite_filepath, metadata_filepath, conus_data_filepath, pixel_radius=5):
